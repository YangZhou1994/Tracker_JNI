...
JNIEXPORT jbyteArray JNICALL Java_org_cripac_isee_vpe_util_Tracking_readConfFile
  (JNIEnv *env , jobject obj , jstring buffer){
	    ...
	    jbyteArray   barr=   (jbyteArray)env->CallObjectMethod(buffer,mid,strencode);
...
JNIEXPORT jobjectArray JNICALL Java_org_cripac_isee_vpe_util_Tracking_getTargets
  (JNIEnv *env , jobject obj , jlong pointer){
		...
		jfieldID locationSequence_id = env ->GetFieldID(class_Track, "locationSequence" , "[Lorg/cripac/isee/vpe/util/Track$BoundingBox;");
	    ...
	    for(int i=0; i<tracks_num; ++i){
	    	...
	    	for(j=0; j<tracks[i].traj_size; ++j){
	    		...
	    		jbyteArray jba = env->NewByteArray(tracks[i].location_sequence[j].width * tracks[i].location_sequence[j].height * 3);
	    		env->SetByteArrayRegion(jba, 0, tracks[i].location_sequence[j].width * tracks[i].location_sequence[j].height * 3, (jbyte*)tracks[i].location_sequence[j].patch_data);
